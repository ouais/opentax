...
def debug_extraction(pdf_path):
    ...
    print(f"✅ Found anchor: '{payer_section.group(0)}' at index {payer_section.start()}")
    ...
    for line in reversed(lines_before[-15:]):
        ...
        if cleaned:
            ...
            # Debug why rejected
            if looks_like_address(line): print("     (Looks like address)")
            for label in FIELD_LABELS:
                if label in line.lower(): print(f"     (Matches label: {label})")
            if re.match(r'^(?:City|State|Zip|Address|Street)\s*:', line, re.IGNORECASE):
