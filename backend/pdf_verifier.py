...
def verify_pdf_semantics(pdf_path):
        ...
        if '/Annots' in page:
            for annot in page['/Annots']:
                ...
                if obj and '/T' in obj and '/V' in obj:
                    key = obj['/T']
                    val = obj['/V']
                    # Handle indirect objects if necessary, usually pypdf resolves /V string
                    fields[key] = str(val)
    ...
    if qual_divs > ord_divs:
        errors.append(f"Semantic Error: Qualified Dividends ({qual_divs}) > Ordinary Dividends ({ord_divs})")
    ...
    if abs(calculated_total - total_income) > 1.0:
        errors.append(f"Semantic Error: Line 9 Total Income ({total_income}) != Sum of components ({calculated_total})")
    ...
    if abs((total_income - adjustments) - agi) > 1.0:
        errors.append(f"Semantic Error: Line 11 AGI ({agi}) != Line 9 ({total_income}) - Line 10 ({adjustments})")
    ...
    if abs(taxable_income - expected_taxable) > 1.0:
        errors.append(f"Semantic Error: Line 15 Taxable Income ({taxable_income}) != Line 11 ({agi}) - Line 12 ({std_deduction})")
    ...
    if abs(total_tax - (tax + other_taxes)) > 1.0:
        errors.append(f"Semantic Error: Line 24 Total Tax ({total_tax}) != Line 16 ({tax}) + Line 23 ({other_taxes})")
    ...
    # Note: Other credits might exist, so Total Payments >= Withheld + Estimated
    if total_payments < (withheld + estimated) - 1.0:
         errors.append(f"Semantic Error: Line 33 Total Payments ({total_payments}) < Withheld ({withheld}) + Estimated ({estimated})")
    ...
    if amount_owed > 0 and overpaid > 0:
        errors.append("Semantic Error: Cannot have both Amount Owed and Overpaid")

    if amount_owed > 0:
        expected_owed = total_tax - total_payments
        if abs(amount_owed - expected_owed) > 1.0:
             errors.append(f"Semantic Error: Line 37 Amount Owed ({amount_owed}) != Total Tax ({total_tax}) - Payments ({total_payments})")

    if overpaid > 0:
        expected_overpaid = total_payments - total_tax
        if abs(overpaid - expected_overpaid) > 1.0:
             errors.append(f"Semantic Error: Line 34 Overpaid ({overpaid}) != Payments ({total_payments}) - Total Tax ({total_tax})")
