...
FORM_1040_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "forms", "f1040.pdf")

def generate_1040(tax_summary, pii):
    if not os.path.exists(FORM_1040_PATH):
        raise FileNotFoundError(f"Form 1040 PDF template not found at {FORM_1040_PATH}")
    ...
    qualified_divs = tax_summary.get('federal', {}).get('qualified_dividends', 0) or 0
    ...
    other_income = fed.get('gross_income', 0) - (wages + interest + dividends + cap_gains)
    if other_income < 0.01: other_income = 0
    
    print(f"[PDF DEBUG] total_dividends={dividends}, qualified_dividends={qualified_divs}, line_3b={line_3b}", flush=True)
    ...
    fields = {
        ...
        'topmostSubform[0].Page1[0].f1_59[0]': fmt(
            max(dividends, tax_summary.get('federal', {}).get('qualified_dividends', 0) or 0)
        ),
        # Line 7 Capital Gain
...
FORM_540_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "forms", "ca540.pdf")
...
def generate_540(tax_summary, pii):
    ...
    if not os.path.exists(FORM_540_PATH):
        raise FileNotFoundError(f"CA Form 540 PDF template not found at {FORM_540_PATH}")
    ...
    # CA AGI (simplified: same as Federal AGI for residents with no CA adjustments)
    ca_agi = federal_agi
