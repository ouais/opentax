    ...
    try:
        with pdfplumber.open(pdf_path) as pdf:
            ...
            def extract_money(pattern, text):
                ...
                if match:
                    val_str = match.group(1).replace(',', '').replace('$', '').replace('(', '-').replace(')', '')
                    try:
            ...
            # OR "1z" ... amount
            result['wages'] = extract_money(r'1z\s.*?'+money_pattern, full_text)
            
            # Line 24: Total Tax. "24 Add lines 22 and 23... 24 310,221."
            result['total_tax'] = extract_money(r'24\s+Add\s+lines\s+22\s+and\s+23.*?'+money_pattern, full_text)
            
            # Line 25d: Total Federal Withholding
            # "25d Add lines 25a through 25c... 277,103."
            result['federal_withheld'] = extract_money(r'25d\s+Add\s+lines\s+25a\s+through\s+25c.*?'+money_pattern, full_text)
            
            # Line 26: 2024 estimated tax payments and amount applied from 2023 return
            result['estimated_tax_payments'] = extract_money(r'26\s+2024\s+estimated\s+tax\s+payments.*?'+money_pattern, full_text)
            
            # Line 25c: Other forms
            # This is tricky because it's usually inside the block 25.
            # "25c Other forms (see instructions)..."
            match_25c = re.search(r'25c\s+Other\s+forms.*?'+money_pattern, full_text)
            if match_25c:
                 result['other_withholding'] = float(match_25c.group(1).replace(',', '').replace('$', ''))
            
            # Line 37: Amount you owe
            result['amount_owed'] = extract_money(r'37\s+Amount\s+you\s+owe.*?'+money_pattern, full_text)
            
            # Confidence check
            # If we found Total Tax or Amount Owed, fairly confident it's a 1040
            ...
            # If main.py sums W-2 parser (Line 25a) AND 1040 parser (Line 25d), WE DOUBLE COUNT!
            ...
            # Let's keep it in result but name it 'validation_total_withheld' so it doesn't get summed into 'w2_federal_withheld'
            result['validation_total_withheld'] = result.pop('federal_withheld')
