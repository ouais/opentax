...
def looks_like_address(text: str) -> bool:
    ...
    if any(w in STREET_SUFFIXES for w in words):
        # To be safe, require at least one digit in the string OR it matches specific pattern
...
def clean_name(name: str) -> str:
    ...
    if re.match(r'^(?:City|State|Zip|Address|Street)\s*:', stripped, re.IGNORECASE):
        return ''

    # Try to strip common Name/Payer prefixes if they exist with a value
    # e.g. "Name Line 1: Vanguard" -> "Vanguard"
    match = re.match(r'^(?:Name\s*Line\s*\d+|Payer(?:\'?s)?(?:\s*Name)?|Employer(?:\'?s)?(?:\s*Name)?|Recipient(?:\'?s)?(?:\s*Name)?)\s*[:.]?\s*(.+)', stripped, re.IGNORECASE)
    if match:
    ...
    # Use word boundary check for short labels to avoid false positives (e.g. 'tin' in 'Marketing')
    for label in FIELD_LABELS:
        if len(label) <= 4:
            ...
            if label in lower:
                ...
                # If label is 'financial institution', it won't match 'Vanguard Financial Services'.
                if label in ['financial institution', 'tax form', 'statement']:
                     if label in lower: return ''
                elif label in lower:
                    return ''
        ...
        for page in pdf.pages:
            ...
            for annot in (page.annots or []):
                ...
                if field_val and ('payer' in field_name or 'employer' in field_name or 'name' in field_name):
                    ...
                    if any(x in field_name for x in ['address', 'addr', 'city', 'state', 'zip', 'street']):
                        continue
...
def is_company_name(text: str) -> bool:
    ...
    for suffix in ENTITY_SUFFIXES:
        if lower.endswith(' ' + suffix) or lower.endswith(',' + suffix) or lower == suffix:
            return True
...
def extract_payer_name_from_text(full_text: str, label_pattern=r'(?:PAYER[^\n]{0,30}name|Name\s*Line\s*1)') -> str:
    ...
    if payer_section:
        ...
        # If we have a candidate below, it's likely the intended name in a standard form
        if candidates_below:
