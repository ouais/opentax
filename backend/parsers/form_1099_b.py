...
def parse_1099_b(pdf_path: str) -> dict:
    ...
    try:
        with pdfplumber.open(pdf_path) as pdf:
            ...
            short_term_patterns = [
                r'Short[\-\s]?term.*?total[s]?.*?' + money_pattern,
                r'Total\s+short[\-\s]?term.*?' + money_pattern,
                r'Net\s+short[\-\s]?term\s+capital\s+gain\s+or\s+\(loss\).*?' + money_pattern,
            ]
            
            long_term_patterns = [
                r'Long[\-\s]?term.*?total[s]?.*?' + money_pattern,
                r'Total\s+long[\-\s]?term.*?' + money_pattern,
                r'Net\s+long[\-\s]?term\s+capital\s+gain\s+or\s+\(loss\).*?' + money_pattern,
            ]
            ...
            for pattern in long_term_patterns:
                matches = re.findall(pattern, full_text, re.IGNORECASE | re.DOTALL)
                if matches:
                    value_str = matches[-1].replace(',', '').replace('$', '').replace('(', '-').replace(')', '')
                    try:
            ...
            withheld_match = re.search(withheld_pattern, full_text, re.IGNORECASE)
            if withheld_match:
