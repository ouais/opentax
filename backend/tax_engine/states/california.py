    ...
    2024: {
        ...
        'joint': {
            'brackets': [
                ...
                (80490, 0.04),
                (111732, 0.06),
                # Wait, search result said 142976 for 9.3% start.
                # 70606*2=141212.
                (141212, 0.08),
...
def calculate_california_tax(
    ...
    # Calculate gross income - California taxes nearly everything as ordinary
    # income
    gross_income = (
        wages +
        interest_income +
...
class CaliforniaStateCalculator(StateTaxCalculator):
    def calculate(self, tax_input: StateTaxInput) -> StateTaxResult:
        ...
        res = calculate_california_tax(
            wages=tax_input.get(
                'wages', 0.0), interest_income=tax_input.get(
                'interest_income', 0.0), dividend_income=tax_input.get(
                'dividend_income', 0.0), capital_gains=tax_input.get(
                    'capital_gains', 0.0), self_employment_income=tax_input.get(
                        'self_employment_income', 0.0), tax_year=tax_input.get(
                            'tax_year', 2024), filing_status=tax_input.get(
    ...
    def get_standard_deduction(
        ...
        return rates[filing_status.lower()]['standard_deduction']
