    ...
    for case in cases:
        ...
        try:
            ...
            if 'foreign_income' in data:
                expected_gross = data.get('w2_wages', 0) + data.get('foreign_income', 0) + data.get('interest_income', 0)
                actual_gross = result['federal']['gross_income']
                if actual_gross < expected_gross - 1.0:
                    raise ValueError(f"Foreign Income not in Gross. Expected >={expected_gross}, Got {actual_gross}")
            ...
            if 'itemized_deductions' in data:
                ...
                if itemized > std:
                    ...
                    if abs(actual_taxable - expected_taxable) > 1.0:
                        raise ValueError(f"Itemized not applied. Expected Taxable {expected_taxable}, Got {actual_taxable}")
                else:
                    # Expect Standard
                    expected_taxable = max(0, agi - std)
                    if abs(actual_taxable - expected_taxable) > 1.0:
                         raise ValueError(f"Standard deduction check failed. Expected {expected_taxable}, Got {actual_taxable}")

            # 4. State Safety
            state = data.get('state', 'CA')
            # Use standardized key first, fall back to legacy
            cal_res = result['california']
            state_tax = cal_res.get('total_state_tax', cal_res.get('total_california_tax', 0.0)) 
            
            NO_TAX_STATES = ['TX', 'FL', 'WA', 'TN', 'NV', 'SD', 'WY', 'AK', 'NH']
            
            if state in NO_TAX_STATES:
                if state_tax != 0.0:
                     raise ValueError(f"No-Tax State {state} has tax: {state_tax}")
            elif state == 'NY':
                wages = data.get('w2_wages', 0)
                # NY Std Ded is 8000. If wages > 8000, verify tax > 0
                if wages > 10000 and state_tax == 0.0:
                      raise ValueError(f"NY should have tax for ${wages} wages, got 0.0")
            elif state == 'CA':
                 wages = data.get('w2_wages', 0)
            ...
            # Mock PII
            pii = {
                'firstName': "TEST", 'lastName': "CASE_" + case['id'],
                'ssn': "000-00-0000",
                'address': "123 Test St", 'city': "City", 'state': "CA", 'zip': "90000"
            }
            ...
            if wages > 0:
                ...
                if not found_wages_field:
                     raise ValueError(f"Wages value found, but NOT in f1_47. Found in: {[f.get('/T') for f in fields]}")
            ...
            if interest > 0:
                ...
                if not found_interest:
                     raise ValueError(f"Interest value found, but NOT in f1_57. Found in: {[f.get('/T') for f in fields]}")
            ...
            if divs > 0:
                ...
                for f in fields:
                    ...
                    if 'f1_59' in name:
                        ...
                        break
                if not found_divs:
                     raise ValueError(f"Ordinary Dividend value ({divs}) found, but NOT in f1_59. Found in: {[f.get('/T') for f in fields]}")
            taxable = result['federal']['taxable_income']
            if taxable > 0:
                fields = find_fields_by_value(taxable)
                if not fields:
                     raise ValueError(f"Taxable Income {taxable} NOT FOUND in PDF.")
                ...
                if not found_taxable:
                     raise ValueError(f"Taxable Income value found, but NOT in f2_06. Found in: {[f.get('/T') for f in fields]}")
            ...
            if semantic_errors:
                ...
                for err in semantic_errors:
                    print(f"    - {err}")
                raise ValueError(f"Semantic Validation Failed: {semantic_errors[0]}")
