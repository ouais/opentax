...
def test_1040_field_mapping_and_layout():
    ...
    assert 'f1_57' in interest_name, f"Interest in wrong field: {interest_name}"
    ...
    # Dividends (3b) -> Expect f1_59 (uses federal total_ordinary_dividends)
    div_field = find_field_by_value(
        summary['federal']['total_ordinary_dividends'])
    assert div_field, "Dividends value not found"
    div_name = div_field.get('/T', 'Unknown')
    assert 'f1_59' in div_name, f"Dividends in wrong field: {div_name}"
    div_y = div_field['/Rect'][1]

    # Assert Interest ABOVE Dividends
    assert interest_y > div_y, "Interest should be above Dividends"

    # Taxable Income (15) -> Expect f2_06
    ...
    assert 'f2_06' in taxable_name, f"Taxable Income in wrong field: {taxable_name}"

    # Total Tax (24) -> Expect f2_16
    # The summary has total_federal_tax = 99999.0.

    total_tax_field = find_field_by_value(
        summary['federal']['total_federal_tax'])
    assert total_tax_field, "Total Tax not found"
    tax_name = total_tax_field.get('/T', 'Unknown')
    assert 'f2_16' in tax_name, f"Tax in wrong field: {tax_name}"
...
def test_3a_3b_dividend_split():
    ...
    assert get_field_value('f1_58') == '1000.00', \
        f"Case 1: Line 3a expected '1000.00', got '{get_field_value('f1_58')}'"
    assert get_field_value('f1_59') == '3000.00', \
        f"Case 1: Line 3b expected '3000.00', got '{get_field_value('f1_59')}'"
    ...
    assert get_field_value2('f1_58') == '656.22', \
        f"Case 2: Line 3a expected '656.22', got '{get_field_value2('f1_58')}'"
    assert get_field_value2('f1_59') == '656.22', \
        f"Case 2: Line 3b expected '656.22', got '{get_field_value2('f1_59')}'"
