...
# backend/tests/states/california)
...
class TestCaliforniaTax(unittest.TestCase):
    ...
    def test_capital_gains_are_ordinary(self):
        ...
        res_cap = self.calc.calculate(
            {'capital_gains': 50000, 'tax_year': 2024})

        # Tax should be identical for same dollar amount
        self.assertAlmostEqual(
            res_wages['total_state_tax'],
            res_cap['total_state_tax'],
            places=2)
