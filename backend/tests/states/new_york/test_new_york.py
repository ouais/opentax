...
# Add backend to path (3 levels up to reach backend/ from
# backend/tests/states/new_york)
...
class TestNewYorkTax(unittest.TestCase):
    ...
    def test_standard_deduction_2024(self):
        ...
        self.assertEqual(
            self.calc.get_standard_deduction(
                'SINGLE', 2024), 8000.0)
    ...
    def test_federal_agi_fallback(self):
        ...
        res_direct = self.calc.calculate(
            {'federal_agi': 50000, 'tax_year': 2024})
        res_components = self.calc.calculate(
            {'wages': 50000, 'tax_year': 2024})

        self.assertAlmostEqual(
            res_direct['total_state_tax'],
            res_components['total_state_tax'],
            places=2)
